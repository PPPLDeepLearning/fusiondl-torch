...
from data import Signal, ProfileSignal, ChannelSignal, Machine
...
d3d = Machine(
    "d3d", "atlas.gat.com", fetch_d3d_data, max_cores=32, current_threshold=2e-1
)
jet = Machine(
    "jet", "mdsplus.jet.efda.org", fetch_jet_data, max_cores=8, current_threshold=1e5
)
...
# superset of shots.
ip = Signal(
    "plasma current ipsip", ["jpf/da/c2-ipla", "/ipsip"], [jet, d3d], is_ip=True
)

ipori = Signal(
    "plasma current", ["jpf/da/c2-ipla", "/ipspr15V"], [jet, d3d], is_ip=True
)
...
iptargett = Signal(
    "plasma current target tol", ["/ipsiptargt"], [d3d], data_avail_tolerances=[0.029]
)
iperr = Signal("plasma current error", ["/ipeecoil"], [d3d])
iperrt = Signal(
    "plasma current error tol", ["/ipeecoil"], [d3d], data_avail_tolerances=[0.029]
)
...
lm = Signal("Locked mode amplitude", ["jpf/da/c2-loca", "/dusbradial"], [jet, d3d])
lmt = Signal(
...
energy = Signal("stored energy", ["jpf/gs/bl-wmhd<s", "/efswmhd"], [jet, d3d])
energyt = Signal(
    "stored energy tol",
...
pin = Signal("Input Power (beam for d3d)", ["jpf/gs/bl-ptot<s", "/bmspinj"], [jet, d3d])
pint = Signal(
    "Input Power (beam for d3d) tol",
...
pradtott = Signal(
    "Radiated Power tol", ["jpf/db/b5r-ptot>out"], [jet], data_avail_tolerances=[0.029]
)
...
pradcore = ChannelSignal(
    "Radiated Power Core", ["ppf/bolo/kb5h/channel14", "/" + r"\bol_l15_p"], [jet, d3d]
)
pradedge = ChannelSignal(
    "Radiated Power Edge", ["ppf/bolo/kb5h/channel10", "/" + r"\bol_l03_p"], [jet, d3d]
)
...
betant = Signal(
    "Normalized Beta tol", ["/efsbetan"], [d3d], data_avail_tolerances=[0.029]
)
energydt = Signal("stored energy time derivative", ["jpf/gs/bl-fdwdt<s"], [jet])

torquein = Signal("Input Beam Torque", ["/bmstinj"], [d3d])
torqueint = Signal(
    "Input Beam Torque tol", ["/bmstinj"], [d3d], data_avail_tolerances=[0.029]
)
tmamp1 = Signal("Tearing Mode amplitude (rotating 2/1)", ["/nssampn1l"], [d3d])
tmamp2 = Signal("Tearing Mode amplitude (rotating 3/2)", ["/nssampn2l"], [d3d])
tmfreq1 = Signal("Tearing Mode frequency (rotating 2/1)", ["/nssfrqn1l"], [d3d])
tmfreq2 = Signal("Tearing Mode frequency (rotating 3/2)", ["/nssfrqn2l"], [d3d])
ipdirect = Signal("plasma current direction", ["/iptdirect"], [d3d])
ipdirectt = Signal(
    "plasma current direction tol", ["/iptdirect"], [d3d], data_avail_tolerances=[0.029]
)
...
d3d_signals = {
    sig_name: sig
    for (sig_name, sig) in all_signals_restricted.items()
    if (sig.is_defined_on_machine(d3d))
}
d3d_signals_n1rms = {
    sig_name: sig
    for (sig_name, sig) in all_signals_n1rms.items()
    if (sig.is_defined_on_machine(d3d))
}
d3d_signals_gar18 = {
    sig_name: sig
    for (sig_name, sig) in all_signals_gar18.items()
    if (sig.is_defined_on_machine(d3d))
}
d3d_signals_0D = {
    sig_name: sig
    for (sig_name, sig) in all_signals_restricted.items()
    if ((sig.is_defined_on_machine(d3d) and sig.num_channels == 1))
}
d3d_signals_1D = {
    sig_name: sig
    for (sig_name, sig) in all_signals_restricted.items()
...
jet_signals = {
    sig_name: sig
    for (sig_name, sig) in all_signals_restricted.items()
    if (sig.is_defined_on_machine(jet))
}
jet_signals_0D = {
    sig_name: sig
    for (sig_name, sig) in all_signals_restricted.items()
    if ((sig.is_defined_on_machine(jet) and sig.num_channels == 1))
}
jet_signals_1D = {
    sig_name: sig
    for (sig_name, sig) in all_signals_restricted.items()
