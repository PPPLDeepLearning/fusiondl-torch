...
class PerformanceAnalyzer:
    ...
    def get_p_thresh_range(self):
        if np.any(self.p_thresh_range) is None:
            ...
            (
                ...
                late_th_tr,
                nd_th_tr,
            ) = self.get_threshold_arrays(
                all_preds_tr, all_truths_tr, all_disruptive_tr
            )
            (
                early_th_te,
                correct_th_te,
                ...
                all_preds_te, all_truths_te, all_disruptive_te
            )
    ...
    def get_metrics_vs_p_thresh_fast(self, all_preds, all_truths, all_disruptive):
        ...
        if self.pred_train is not None:
            ...
            p_thresh_range = np.sort(
                np.concatenate((early_th, correct_th, late_th, nd_th))
            )
        correct_range = np.zeros_like(p_thresh_range)
        accuracy_range = np.zeros_like(p_thresh_range)
        fp_range = np.zeros_like(p_thresh_range)
    ...
    def get_shot_prediction_stats_from_threshold_arrays(
        ...
        lates = np.sum(
            np.logical_and(
                np.logical_and(early_th <= thresh, correct_th <= thresh),
                late_th > thresh,
            )
        )
        FNs = np.sum(
    ...
    def get_shot_difficulty(self, preds, truths, disruptives):
        ...
        nd_ranks = stats.rankdata(
            -nd_thresholds, method="min"
        )  # difficulty is highest when threshold is high, can't avoid false positive
    ...
    def get_threshold_arrays(self, preds, truths, disruptives):
        ...
        return (
            np.array(d_early_thresholds),
            np.array(d_correct_thresholds),
            np.array(d_late_thresholds),
            np.array(nd_thresholds),
        )
    ...
    def summarize_shot_prediction_stats(
        self, P_thresh, all_preds, all_truths, all_disruptive, verbose=False
    ):
    ...
    def get_positives(self, predictions):
        ...
        return np.where(
            np.logical_and(predictions, indices >= self.get_ignore_indices())
        )[0]
    ...
    def load_ith_file(self):
        ...
        print(results_files)
        print(
            "Loading results file {}".format(self.results_dir + results_files[self.i])
        )
        dat = np.load(self.results_dir + results_files[self.i], allow_pickle=True)
        ...
        for mode in ["test", "train"]:
            print(
                "{}: loaded {} shot ({}) disruptive".format(
                    mode, self.get_num_shots(mode), self.get_num_disruptive_shots(mode)
                )
            )
        if self.verbose:
            self.print_conf()
    ...
    def hist_alarms(self, alarms, title_str="alarms", save_figure=False, linestyle="-"):
        ...
        if len(alarms) > 0:
            ...
            #
            plt.step(
                np.concatenate((alarms[::-1], alarms[[0]])),
    ...
    def compute_tradeoffs_and_print(self, mode):
        ...
        (
            ...
            missed_range,
            early_alarm_range,
        ) = self.get_metrics_vs_p_thresh(mode)
        ...
        for fp_thresh in fp_threshs:
            ...
            if any(fp_range < fp_thresh):
                ...
                print(
                    "============= AT P_THRESH = {} =============".format(P_thresh_opt)
                )
            else:
    ...
    def compute_tradeoffs_and_print_from_training(self):
        ...
        (
            ...
            missed_range,
            early_alarm_range,
        ) = self.get_metrics_vs_p_thresh("train")
    ...
    def compute_tradeoffs_and_plot(
        self, mode, save_figure=True, plot_string="", linestyle="-", thres_opt=None
    ):
        (
            correct_range,
            accuracy_range,
    ...
    def plot_individual_shot(
        self, P_thresh_opt, shot_num, normalize=True, plot_signals=True
    ):
        success = False
        for mode in ["test", "train"]:
            if mode == "test":
    ...
    def example_plots(
        ...
        for i in iterate_arr:
            ...
            prediction_type = self.get_prediction_type(TP, FP, FN, TN, early, late)
            if not all(
                _ in set(["FP", "TP", "FN", "TN", "late", "early", "any"])
                for _ in types_to_plot
            ):
                print("warning, unkown types_to_plot")
                return
            if (
                "any" in types_to_plot or prediction_type in types_to_plot
    ...
    def plot_shot(
        ...
        if self.normalizer is None and normalize:
            if self.conf is not None:
                self.saved_conf["paths"]["normalizer_path"] = self.conf["paths"][
                    "normalizer_path"
                ]
        ...
        if shot.previously_saved(self.shots_dir):
            ...
            if shot.signals_dict is not None:  # make sure shot was saved with data
                ...
                ax.axhline(
                    P_thresh_opt, linestyle="--", color="k", label="threshold", zorder=2
                )
                ax.axvline(p0, linestyle="--", color="darkgrey")
    ...
    def plot_shot_old(
        ...
        if shot.previously_saved(self.shots_dir):
            ...
            if len(truth) - self.T_max_warn >= 0:
                ax.axvline(
                    len(truth) - self.T_max_warn, color="r", label="min warning time"
                )
            ax.axvline(
                len(truth) - self.T_min_warn, color="r", label="max warning time"
            )
            ax.set_xlabel("T [ms]")
    ...
    def tradeoff_plot(
        ...
        if self.pred_ttd:
            ...
            plt.plot(
                P_thresh_range, missed_range, "r", label="missed", linestyle=linestyle
            )
            plt.plot(
                P_thresh_range,
                fp_range,
