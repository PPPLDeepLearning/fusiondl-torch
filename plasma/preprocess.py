...
from downloading import mkdirdepth
...
class Preprocessor(object):
    ...
    def clean_shot_lists(self):
        ...
        paths = [
            os.path.join(shot_list_dir, f)
            for f in listdir(shot_list_dir)
            if os.path.isfile(os.path.join(shot_list_dir, f))
        ]
        for path in paths:
    ...
    def preprocess_from_files(self, shot_files, use_shots):
        ...
        for (i, shot) in enumerate(
            pool.imap_unordered(self.preprocess_single_file, shot_list_picked)
        ):
            # for (i,shot) in
    ...
    def preprocess_single_file(self, shot):
        ...
        if recompute or not shot.previously_saved(processed_prepath):
            ...
            try:
                ...
                shot.save(processed_prepath)
                sys.stdout.write(
                    "\r{} exists but corrupted, resaved.".format(shot.number)
                )
        shot.make_light()
        return shot
    ...
    def load_shotlists(self):
        ...
        if isinstance(shot_list_train, ShotList):
            ...
            return (
                ShotList(shot_list_train),
                ShotList(shot_list_validate),
                ShotList(shot_list_test),
            )
...
def apply_bleed_in(conf, shot_list_train, shot_list_validate, shot_list_test):
    ...
    if num > 0:
        ...
        assert (
            num_d >= num
        ), "Not enough disruptive shots {} to cover bleed in {}".format(num_d, num)
        num_sampled_d = 0
        num_sampled_nd = 0
        while num_sampled_d < num:
            ...
            if s.is_disruptive:
                ...
                num_sampled_nd += 1
        print(
            "Sampled {} shots, {} disruptive, {} nondisruptive".format(
                num_sampled_nd + num_sampled_d, num_sampled_d, num_sampled_nd
            )
        )
        print(
            "Before adding: training shots: {} validation shots: {}".format(
        ...
        if conf["data"]["bleed_in_equalize_sets"]:
            ...
            for s in shot_list_bleed:
                ...
                shot_list_validate.append(s)
        print(
            "After adding: training shots: {} validation shots: {}".format(
                len(shot_list_train), len(shot_list_validate)
            )
        )
...
def guarantee_preprocessed(conf, verbose=False):
    ...
    if verbose:
        ...
        g.print_unique(
            "testing: {} shots, {} disruptive".format(
