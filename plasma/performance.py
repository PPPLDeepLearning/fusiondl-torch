...
class PerformanceAnalyzer:
    ...
    def compute_tradeoffs_and_plot(
        self, mode, save_figure=True, plot_string="", linestyle="-"
    ):
        (
            correct_range,
            accuracy_range,
        ...
        if shot.previously_saved(self.shots_dir):
            ...
            if shot.signals_dict is not None:
                ...
                for i, sig in enumerate(use_signals):
                    ...
                    ax.legend(
                        loc="best", labelspacing=0.1, fontsize=fontsize, frameon=False
                    )
                ...
                if save_fig:
                    plt.savefig("sig_fig_{}{}.png".format(shot.number, extra_filename)),
                    #        bbox_inches='tight')
                    np.savez(
