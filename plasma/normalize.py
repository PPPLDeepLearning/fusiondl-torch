...
class Normalizer(object):
    ...
    def train(self, verbose=False):
        ...
        if train_machines >= all_machines:
            ...
            print(
                "Testing set contains new machine, using testing set ",
        ...
        return self.train_on_files(
            shot_files_use, use_shots, all_machines, verbose=verbose
        )

    def train_on_files(self, shot_files, use_shots, all_machines, verbose=False):
        conf = self.conf
        ...
        if not previously_saved or len(machines_to_compute) > 0:
            ...
            for (i, stats) in enumerate(
                pool.imap_unordered(self.train_on_single_shot, shot_list_picked)
            ):
                # for (i,stats) in
    ...
    def cut_end_of_shot(self, shot):
        ...
        if not self.inference_mode and cut_shot_ends:
            ...
            if shot.ttd.shape[0] - T_min_warn <= max(self.conf["model"]["length"], 0):
                print(
                    "not cutting shot; length of shot after cutting by ",
...
class MeanVarNormalizer(Normalizer):
    ...
    def extract_stats(self, shot):
        ...
        if shot.valid:
            ...
            stats.means = np.reshape(
                np.array([np.mean(sig) for sig in list_of_signals]), (1, num_signals)
            )
            stats.stds = np.reshape(
                np.array([np.std(sig, dtype=np.float64) for sig in list_of_signals]),
                (1, num_signals),
            )
    ...
    def apply(self, shot):
        ...
        m = shot.machine
        assert (
            self.means[m] is not None and self.stds[m] is not None
...
class VarNormalizer(MeanVarNormalizer):
    def apply(self, shot):
        apply_positivity(shot)
        assert (
            self.means is not None and self.stds is not None
        ), "self.means or self.stds not initialized"
        m = shot.machine
        stds = np.median(self.stds[m], axis=0)
        for (i, sig) in enumerate(shot.signals):
...
class MinMaxNormalizer(Normalizer):
    ...
    def incorporate_stats(self, stats):
        ...
        if stats.valid:
            ...
            if self.num_processed == 0:
                ...
                self.minimums[m] = (
                    self.num_processed[m] * self.minimums + minimums
                ) / (self.num_processed[m] + 1.0)
                self.maximums[m] = (
                    self.num_processed[m] * self.maximums + maximums
                ) / (self.num_processed[m] + 1.0)
    ...
    def apply(self, shot):
        ...
        for (i, sig) in enumerate(shot.signals):
            if sig.normalize:
                shot.signals_dict[sig] = (shot.signals_dict[sig] - self.minimums[m]) / (
