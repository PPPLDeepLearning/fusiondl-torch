...
class Loader(object):
            ...
            for (i, shot_sublist) in enumerate(shot_sublists):
                ...
                for j, (X, y) in enumerate(zip(X_list, y_list)):
                    ...
                    for k in range(num_chunks):
                        ...
                        num_so_far += (
                            1.0 * len(shot_sublist) / (len(X_list) * num_chunks)
                        )
    ...
    def fill_training_buffer(
        self, Xbuff, Ybuff, end_indices, shot, is_first_fill=False
    ):
    ...
    def inference_batch_generator_full_shot(self, shot_list):
        ...
        Maskbuff = np.zeros(
            (batch_size,) + res.shape, dtype=self.conf["data"]["floatx"]
        )
    ...
    def load_as_X_y_list(self, shot_list, verbose=False, prediction_mode=False):
        ...
        (signals, results, total_length) = self.get_signals_results_from_shotlist(
            shot_list
        )
        ...
        if self.verbose:
            print(
                "multiplication factor: {}".format(
                    1.0 * effective_length / total_length
                )
            )
            print(
                "effective/total length : {}/{}".format(effective_length, total_length)
            )
            print(
                "patch length: {} num patches: {}".format(
    ...
    def load_as_X_y_pred(self, shot_list, verbose=False, custom_batch_size=None):
        (
            ...
            shot_lengths,
            disruptive,
        ) = self.get_signals_results_from_shotlist(shot_list, prediction_mode=True)
    ...
    def get_signal_result_from_shot(self, shot, prediction_mode=False):
        ...
        if self.normalizer is not None:
            ...
            print(
                "Warning, no normalization. ", "Training data may be poorly conditioned"
            )
    ...
    def make_deterministic_patches(self, signals, results):
        ...
        for sig, res in zip(signals, results):
            (sig_patch, res_patch) = self.make_deterministic_patches_from_single_array(
                sig, res, min_len
            )
    ...
    def get_min_len(self, arrs, length):
        min_len = min(
            [len(a) for a in arrs] + [self.conf["training"]["max_patch_length"]]
        )
    ...
    def make_patches(self, signals, results):
        ...
        if self.verbose:
            print(
                "random to deterministic ratio: {}/{}".format(
                    num_additional, num_already
                )
            )
    ...
    def arange_patches_single(
        self, sig_patches, res_patches, prediction_mode=False, custom_batch_size=None
    ):
        if prediction_mode:
            num_timesteps = self.conf["model"]["pred_length"]
            batch_size = self.conf["model"]["pred_batch_size"]
        ...
        for chunk_idx in range(num_chunks):
            ...
            for patch_idx in range(batch_size):
                X[chunk_idx * batch_size + patch_idx, :, :] = sig_patches[patch_idx][
                    src_start:src_end
                ]
                if return_sequences:
                    y[chunk_idx * batch_size + patch_idx, :, :] = res_patches[
                        patch_idx
                    ][src_start:src_end]
                else:
                    y[chunk_idx * batch_size + patch_idx, :] = res_patches[patch_idx][
                        src_end - 1
                    ]
        return X, y
