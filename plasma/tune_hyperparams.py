...
t_warn = CategoricalHyperparam(["data", "T_warning"], [30, 50, 20.024, 8, 5, 10.024])
cut_ends = CategoricalHyperparam(["data", "cut_shot_ends"], [False, True])
# for shallow
if shallow:
    num_nodes = 1
    shallow_model = CategoricalHyperparam(
        ["model", "shallow_model", "type"], ["svm", "random_forest", "xgboost", "mlp"]
    )
    n_estimators = CategoricalHyperparam(
        ["model", "shallow_model", "n_estimators"], [5, 20, 50, 100, 300, 1000]
    )
    max_depth = CategoricalHyperparam(
        ["model", "shallow_model", "max_depth"], [None, 3, 6, 10, 30, 100]
    )
    C = LogContinuousHyperparam(["model", "shallow_model", "C"], 1e-3, 1e3)
    kernel = CategoricalHyperparam(
        ["model", "shallow_model", "kernel"], ["rbf", "sigmoid", "linear", "poly"]
    )
    ...
    num_samples = CategoricalHyperparam(
        ["model", "shallow_model", "num_samples"], [30000, 100000, 1000000, 2000000]
    )
    ...
    mlp_learning_rate = CategoricalHyperparam(
        ["model", "shallow_model", "learning_rate_mlp"], [0.001, 0.0001, 0.00001]
    )
    ...
    patience = CategoricalHyperparam(
        ["callbacks", "patience"], [15, 25, 60, 75, 100, 200, 300]
    )
    lr_decay_factor = CategoricalHyperparam(
        ["model", "lr_decay_factor"], [1.1, 1.2, 2, 3, 5, 1.5, 10]
    )
    lr_decay_patience = CategoricalHyperparam(
        ["model", "lr_decay_patience"], [3, 5, 6, 8, 10]
    )
    ...
    batch_size = CategoricalHyperparam(["training", "batch_size"], [32, 64, 128, 256])
    noise = CategoricalHyperparam(["training", "noise"], [0.0, 0.01, 0.05, 0.1, 0.08])
    tcn_hidden = CategoricalHyperparam(
        ["model", "tcn_hidden"], [15, 20, 30, 40, 50, 60]
    )
    tcn_layers = CategoricalHyperparam(
        ["model", "tcn_layers"], [7, 8, 9, 10, 11, 12, 13]
    )
    kernel_size_temporal = CategoricalHyperparam(
        ["model", "kernel_size_temporal"], [3, 5, 7, 8, 9, 10, 11, 12, 13]
    )

    tt_lstm_hidden = CategoricalHyperparam(
        ["model", "tt_lstm_hidden"], [8, 10, 15, 20, 30, 50, 100, 200]
    )
    cell_order = CategoricalHyperparam(["model", "cell_order"], [2, 3, 4, 5])
    cell_steps = CategoricalHyperparam(["model", "cell_steps"], [1, 2, 3, 4, 5, 6, 7])
    cell_rank = CategoricalHyperparam(["model", "cell_rank"], [1, 2, 3, 4, 5])
    ...
    rnn_size = CategoricalHyperparam(
        ["model", "rnn_size"], [16, 32, 48, 50, 64, 72, 80, 96, 128]
    )
    dense_size = CategoricalHyperparam(["model", "dense_size"], [64, 128, 200, 160])
    simple_conv = CategoricalHyperparam(["model", "simple_conv"], [False, True])
    model_type = CategoricalHyperparam(["model", "model_type"], ["TTLSTM"])
    ...
    tunables += [simple_conv, rnn_layers, rnn_size, dense_size]
    tunables += [tcn_hidden, tcn_layers, kernel_size_temporal]
    tunables += [tt_lstm_hidden, cell_steps, cell_order, cell_rank]
    # tunables += [t_min_warn]
# tunables += [cut_ends, t_warn]

run_directory = "{}/{}/hyperparams/".format(conf.conf["fs_path"], getpass.getuser())
...
def generate_conf_file(
    tunables, shallow, template_path="../", save_path="./", conf_name="conf.yaml"
):
...
os.system(
    " ".join(["cp -p", os.path.join(template_path, conf_name), working_directory])
)
os.system(
    " ".join(["cp -p", os.path.join(template_path, executable_name), working_directory])
)
os.system(" ".join(["cp -p", os.path.join(template_path, "*.py"), working_directory]))
...
for i in range(num_trials):
    ...
    print("Starting job")
    os.system(" ".join(["cp -p", os.path.join(template_path, "*.py"), subdir]))
    start_slurm_job(
        subdir,
        num_nodes,
        i,
