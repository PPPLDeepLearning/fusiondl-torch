...
class FLSTM(nn.Module):
    def __init__(
        ...
        if device is None:
            ...
            self.device = device
        self.rnn = nn.LSTM(
            self.input_dim, self.rnn_size, batch_first=True, num_layers=self.rnn_layers
        ).to(self.device)
...
class FTCN(nn.Module):
    def __init__(
        ...
        self.tcn = TCN(
            linear_size, output_size, num_channels_tcn, kernel_size_temporal, dropout
        )
...
def build_torch_model(conf):
    ...
    linear_size = 5
    try:
        num_channels_tcn = [conf["model"]["tcn_hidden"]] * conf["model"][
            "tcn_layers"
        ]  # [3]*5
        ...
        num_channels_tcn = [
            40
        ] * 5  # [conf['model']['tcn_hidden']]*conf['model']['tcn_layers']#[3]*5
    ...
    if model_type == "TCN":
        ...
        model = ConvLSTMNet(
            ...
            skip_stride=None,
            cell="convttlstm",
            cell_params={"order": cell_order, "steps": cell_steps, "rank": cell_rank},
            kernel_size=1,
            bias=True,
            output_sigmoid=True,
        )
...
def get_signal_dimensions(conf):
    ...
    for sig in use_signals:
        ...
        if num_channels > 1:
            ...
            assert (
                not is_1D_region
            ), "make sure all use_signals are ordered such that 1D signals come last!"
...
def get_model_path(conf):
    return (
        conf["paths"]["model_save_path"] + model_filename
...
def train(conf, shot_list_train, shot_list_validate, loader):
    ...
    while e < num_epochs - 1:
        ...
        _, _, _, roc_area, loss = make_predictions_and_evaluate_gpu(
            conf, shot_list_validate, loader, inference_model=train_model, device=device
        )
