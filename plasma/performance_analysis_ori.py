...
analyzer.compute_tradeoffs_and_plot(
    "train", save_figure=save_figure, plot_string="_train", linestyle=linestyle
)
...
# analyzer.example_plots(P_thresh_opt,'test',['FP'],extra_filename='testfp',normalize=normalize)
# analyzer.example_plots(P_thresh_opt,'test',['FN'],extra_filename='testfn',normalize=normalize)
analyzer.example_plots(
    P_thresh_opt,
    "test",
    ["TP"],
    max_plot=1,
    extra_filename="testtp",
    normalize=normalize,
)
# analyzer.example_plots(P_thresh_opt,'test',['late'],extra_filename='testlate',normalize=normalize)
...
analyzer.example_plots(
    ...
    ["TN"],
    max_plot=1,
    extra_filename="traintn",
    normalize=normalize,
)
# analyzer.example_plots(P_thresh_opt,'train',['FP'],extra_filename='trainfp',normalize=normalize)
# analyzer.example_plots(P_thresh_opt,'train',['FN'],extra_filename='trainfn',normalize=normalize)
# analyzer.example_plots(P_thresh_opt,'train',['TP'],extra_filename='traintp',normalize=normalize)
# analyzer.example_plots(P_thresh_opt,'train',['late'],extra_filename='trainlate',normalize=normalize)
...
analyzer.hist_alarms(
    nondisr_alarms, "nondisruptive alarms, P thresh = {}".format(P_thresh_opt)
)
print("{} nondisruptive alarms".format(len(nondisr_alarms)))

analyzer.hist_alarms(
    disr_alarms,
    "disruptive alarms, P thresh = {}".format(P_thresh_opt),
