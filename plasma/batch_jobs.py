...
def create_jenkins_script(
    ...
    filename = "jenkins_{}_{}.cmd".format(
        test_configuration[0], test_configuration[1]
    )  # version of Python and the dataset
...
def start_jenkins_job(
    subdir, num_nodes, executable_name, test_configuration, env_name, env_type
):
    os.system(" ".join(["cp -p", executable_name, subdir]))
...
def start_pbs_job(
    subdir, num_nodes, i, conf, shallow, env_name="frnn", env_type="anaconda"
):
    executable_name, use_mpi = get_executable_name(conf)
    os.system(" ".join(["cp -p", executable_name, subdir]))
    script = create_pbs_script(
        subdir, num_nodes, i, executable_name, use_mpi, env_name, env_type
    )
...
def create_slurm_script(
    ...
    # user = getpass.getuser()
    sbatch_header = create_slurm_header(num_nodes, use_mpi, idx, short=short)
    with open(filepath, "w") as f:
        ...
        if cluster == "traverse":
            ...
            f.write(("module load cudatoolkit/9.2 \n"))
            f.write(("module load cudnn/cuda-9.2/7.6.1 \n"))
            f.write(("module load openmpi/gcc/3.1.4/64 \n"))
            ...
            f.write("module load anaconda3/4.4.0 \n")
            f.write("source activate " + env_name + "\n")
            f.write(("module load cudatoolkit/8.0 \n"))
            f.write(("module load cudnn/cuda-8.0/6.0 \n"))
            f.write(("module load openmpi/cuda-8.0/intel-17.0/2.1.0/64 \n"))
            f.write(("module load intel/17.0/64/17.0.5.239 \n"))
            # f.write('rm -f /tigress/{}/model_checkpoints/*.h5\n'.format(user))
            f.write("cd {}\n".format(subdir))
            f.write("srun python {}\n".format(executable_name))
            f.write('echo "done."')
...
def create_slurm_header(num_nodes, use_mpi, idx, short=False):
    if not use_mpi:
        assert num_nodes == 1
    use_mpi = False
    lines = []
